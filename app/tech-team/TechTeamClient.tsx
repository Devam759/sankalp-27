...
export default function TechTeamClient() {
  const teamMembers: TeamMember[] = [
    ...
    },
    {
      ...
      badge: 'Developer',
      isHead: false,
      image: '/Images/team/manant-srivastava.jpeg?v=3',
    ...
    },
    {
      ...
      description: 'Contributed to conference content management, structured data integration, schema validation, testing, and deployment verification, ensuring accurate information delivery and maintaining the reliability and consistency of the SANKALP\'27 conference platform.',
      skills: [
        'Content Management',
        'Data Integration',
        'Schema Validation',
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
            {teamMembers.map((member, idx) => (
              <motion.div
                ...
                className="flex flex-col justify-between p-7 sm:p-8 bg-[#FCFCFD] border border-slate-200/90 rounded-2xl shadow-xs hover:-translate-y-2 hover:shadow-xl hover:border-slate-300 transition-all duration-300 ease-out"
              >
                <div>
                  {/* TOP HEADER: BADGE */}
                  ...
                  {/* PROFILE IMAGE SECTION (Rounded 20px, soft shadow) */}
                  ...
                  {/* SOCIAL ICONS (Positioned beneath member name and role) */}
                  ...
                  {/* CONCISE DESCRIPTION CONTAINER */}
                  <div className={`min-h-[115px] mb-3 ${member.name === 'Manant Srivastava' ? 'pb-8 sm:pb-9' : 'pb-3'}`}>
                  ...
                  <div className="pt-6 border-t border-slate-200/80">
                    <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-3">
                      {member.skillsHeader || 'CORE EXPERTISE'}
                    </span>
                    <div className="grid grid-cols-2 gap-1.5">
                      {member.skills.map((skill, sIdx) => (
                        <span
                          key={sIdx}
                          className="px-2.5 py-1.5 text-[11px] font-semibold text-slate-700 bg-slate-100/90 border border-slate-200/80 rounded-md flex items-center justify-center text-center leading-tight"
                        >
                          {skill}
                        </span>
                      ))}
                  ...
                  {/* PROJECT OVERSIGHT (For Lead Card only) */}
                  {member.oversight && member.oversight.length > 0 && (
                    <div className="pt-4 mt-3.5 border-t border-slate-200/80">
                      ...
                      <ul className="space-y-1.5">
                        {member.oversight.map((item, oIdx) => (
